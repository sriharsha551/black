...
<h4 class="font-weight-bold py-2 mb-4">
	...
	<?php 
				...
				foreach($amounts as $amount)
					{
						if($amount->id == $GLOBALS['invoice_id'])
						{
...
<div class="card mb-4">
			...
			<div class="card-body"> 
				...
				<div class="col-md-6">
				<label for="prj_id" class="form-label"><span class="text-danger">*</span>Invoice Id</label>
				<div class='form-group'>
				<select class="form-control" name="invoice_id" onchange="this.form.submit();">
							<option value=''>select name</option>
							<?php foreach($inv_ids as $row) {?>
  							<option value='<?php echo $row->id?>' <?php echo ($row->id == $GLOBALS['invoice_id'] || $invoice['id'] == $row->id || $this->input->post('inv_id')== $row->id) ? 'selected="selected"' : "" ?>><?php echo $row->invoice_num?></option>
							<?php }?>
						</select>
					...
					<div class="row clearfix">
						...
						<input type="hidden" name="inv_id" value="<?php if(isset($GLOBALS['invoice_id'])){echo $GLOBALS['invoice_id'];} ?>" class="form-control" id="inv_id" />						
							<span class="text-danger"><?php if($_SESSION['error']==true)echo form_error('inv_id');?></span>
						</div>
						</div>
                        <div class="col-md-6">
							...
							<select class="form-control" name="coa_id" >
							        <?php foreach($coa_ids as $row) {?>
  							        <option value='<?php echo $row->id?>' <?php echo ($row->id == $invoice['coa_id'] || $this->input->post('coa_id') == $row->id ) ? 'selected="selected"' : "" ?> ><?php echo $row->name?></option>
							        <?php }?>
								</select>	
                        	...
                        	<label for="prj_id" class="col-md-6"><span class="text-danger">*</span>Project</label>
								<div class="form-group">
									<select name="prj_id" class="form-control" id="prj_id"  required>
                                		...
                                		<option value="<?php echo $prj['id']; ?>" <?php echo (($this->input->post('prj_id') == $prj['id']) || ($invoice['prj_id'] == $prj['id'])) ? 'selected="selected"' : ""; ?> ><?php echo $prj['name']; ?></option>
                                		<?php } ?>
                           			</select> 
		 						</div>
						</div>
                        <div class="col-md-6">
							...
							<div class="form-group">
								<input type="date" name="paid_dt" value="<?php echo ($this->input->post('paid_dt') ? $this->input->post('paid_dt') : $invoice['paid_dt']); ?>" class="form-control" id="paid_dt" />
                        ...
                        <div class="col-md-6">
							...
							<select class="form-control" name="pay_method" >
							        <?php foreach($pay_ids as $row) {?>
  							        <option value='<?php echo $row->id?>' <?php echo ($row->id == $invoice['pay_method'] || $this->input->post('pay_method') == $row->id) ? 'selected="selected"' : "" ?> ><?php echo $row->name?></option>
							        <?php }?>
								</select>
                        ...
                        <!-- <div class="col-md-6">
							...
							<select class="form-control" name="tran_type_id" >
							        <?php foreach($tran_ids as $row) {?>
  							        <option value='<?php echo $row->id?>' <?php echo ($row->id == $invoice['trans_id'] || $this->input->post('trans_id') == $row->id) ? 'selected="selected"' : "" ?> ><?php echo $row->name?></option>
							        <?php }?>
								</select>
