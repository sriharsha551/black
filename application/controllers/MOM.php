    ...
    { 
        ...
        <div class="col-md-12">
            ...
            <div class="form-group col-md-1">                           
                <a  href="javascript:void(0);"  class="btn btn-danger btn-xs remove_point" title="Remove point"><span class="ion ion-md-close-circle"></span></a>
            </div>
        </div>
        <?php exit;
    }
    ...
    {   
        ...
        if(isset($this->data['mom']['id']))
        {
            ...
            {   
                ...
                //update mom members
                $this->MOM_model->delete_mom_members($id);
                foreach($this->input->post('employees') as $key=>$fname) 
                {  
                    if($this->input->post('employees')[$key]!="")
                    {
                ...
                //update mom points
                $this->MOM_model->delete_mom_points($id);   
                foreach($this->input->post('points') as $key=>$fname) 
                {  
                    if($this->input->post('points')[$key]!="")
                    {
            ...
            else
            {
                $this->data['projects'] = $this->Project_model->get_all_projects();
                $this->data['employees'] = $this->Employees_model->get_all_employee_order_dep();
                $emps = $this->MOM_model->get_all_mom_employees($id);
               // print_r($emps); exit;
                $array_emps = array();
                foreach($emps as $emp)
                {
                ...
                $this->data['mom_points'] = $this->MOM_model->get_all_mom_points($id);
